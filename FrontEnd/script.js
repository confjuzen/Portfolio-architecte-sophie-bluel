...
function rendermain() {
  ...
  p3.textContent =
    "En cas de besoin, une équipe pluridisciplinaire peut-être letituée : architecte DPLG, décorateur(trice)";
...
function modifypopup() {
  ...
  function addpopup() {
    ...
    let backpath = document.createElementNS(
...
function verifyupload() {
  ...
  let errors = [fileerror, titleerror, categoryerror].filter(
    (msg) => msg !== ""
  );
  if (errors.length > 0) {
    if (errors.length === 1) {
